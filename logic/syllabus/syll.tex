...
\vfill
{\small\textcolor{darkgray}{Junio 2026}}
\end{center}
\end{titlepage}
...
%  SESIÓN 7
% ============================================================
\section{Sesi\'on 7 --- Termodin\'amica L\'ogica: El L\'imite Mat\'erico}

\begin{prereqs}
Sesi\'on 6. La dependencia tecnol\'ogica hacia la no-linealidad.
\end{prereqs}

\subsection*{Motivaci\'on}
El dise\~no arquitect\'onico l\'ogico exige un componente capaz de conmutar se\~nales rechazando la entrop\'ia t\'ermica. En lugar de derivar la mec\'anica estad\'istica profunda, nos enfocaremos en la \emph{filosof\'ia f\'isica}: c\'omo la brecha prohibida ($E_g$) a\'isla la conducci\'on del caos termodin\'amico y c\'omo el dopaje rompe la simetr\'ia material para estabilizar el "bit".

\subsection{Contenidos}

\subsubsection{7.1 --- El Cristal y la Brecha Prohibida}
\begin{itemize}
  \item La brecha prohibida (\emph{bandgap}) $E_g$: el umbral de energ\'ia que a\'isla la informaci\'on estructurada del ruido t\'ermico.
  \item La amenaza de la termodin\'amica: c\'omo el incremento de la temperatura tiende a destruir el estado de aislamiento, comprometiendo la viabilidad del c\'omputo.
\end{itemize}

\subsubsection{7.2 --- Ruptura de Simetr\'ia: Dopaje y la Uni\'on PN}
\begin{itemize}
  \item Control material: el dopaje como m\'etodo f\'isico para inducir asimetr\'ia en un cristal el\'ectricamente neutro.
  \item La uni\'on PN como frontera termodin\'amica: creaci\'on de la regi\'on de agotamiento y el campo el\'ectrico de equilibrio.
  \item \textbf{El nacimiento de la no-linealidad:} El diodo no como una ecuaci\'on diferencial abstracta, sino como la primera pieza de materia que logramos obligar a rectificar informaci\'on unidireccionalmente, estabilizando el c\'omputo frente al caos termodin\'amico.
\end{itemize}

\begin{objetivos}
\begin{itemize}
  \item Explicar conceptualmente c\'omo la brecha prohibida protege la integridad l\'ogica del bit f\'isico.
  \item Comprender la necesidad tecnol\'ogica de romper la simetr\'ia el\'ectrica mediante la uni\'on PN para lograr un comportamiento no lineal.
\end{itemize}
\end{objetivos}

\begin{lecturas}
\begin{itemize}
  \item[$(\star)$] \textbf{Streetman, B. G., \& Banerjee, S. K.} --- \emph{Solid State Electronic Devices}. (Enfoque selectivo y conceptual en $E_g$ y la operaci\'on de la uni\'on, omitiendo derivaciones estad\'isticas duras).
  \item \textbf{Feynman, R. P.} --- \emph{Feynman Lectures on Computation}. Cap\'itulo 5 (C\'omputo y termodin\'amica).
\end{itemize}
\end{lecturas}
...
%  SESIÓN 8
% ============================================================
\section{Sesi\'on 8 --- El Modus Ponens F\'isico: Arquitectura CMOS}

\begin{prereqs}
Sesiones 2, 6 y 7. Convergencia del modelo axiom\'atico abstractivo y los dispositivos de ruptura de simetr\'ia.
\end{prereqs}

\subsection*{Motivaci\'on}
La formalizaci\'on axiom\'atica requiere conectivos como el operador NAND. Esta sesi\'on ensambla la tecnolog\'ia CMOS utiliz\'andola como el lienzo final, cerrando la trayectoria desde el axioma metamatem\'atico hasta la f\'isica aplicada mediante la pura topolog\'ia de interruptores.

\subsection{Contenidos}

\subsubsection{8.1 --- El Transistor como Interruptor Ideal}
\begin{itemize}
  \item El MOSFET conceptual: descripci\'on de la separaci\'on funcional estricta entre la se\~nal de control (compuerta) y la v\'ia de conducci\'on (canal).
  \item Abstracci\'on del dispositivo de estado s\'olido a un interruptor controlado por voltaje, eliminando el c\'alculo anal\'itico de saturaci\'on.
\end{itemize}

\subsubsection{8.2 --- La Arquitectura L\'ogica CMOS}
\begin{itemize}
  \item Resoluci\'on topol\'ogica del alto consumo de energ\'ia: c\'omo enga\~nar a las leyes de la termodin\'amica mediante simetr\'ia complementaria.
  \item Ensamble estructural: La evaluaci\'on de la funci\'on $f$ recae en la red de canal N (Pull-Down), mientras la red de canal P (Pull-Up) garantiza mec\'anicamente la entrega del complemento l\'ogico $\neg f$.
\end{itemize}

\subsubsection{8.3 --- S\'intesis Final: Implementaci\'on del NAND Universal}
\begin{itemize}
  \item Taller topol\'ogico: Dise\~no esquem\'atico de la compuerta NAND est\'atica mediante redes de transistores.
  \item \textbf{El arco argumental completo:} Conexi\'on axiom\'atica de la f\'ormula formal $\neg(A \land B)$ con la carga cu\'antica modulada por voltajes. Demostraci\'on final de c\'omo atrapamos la completitud l\'ogica de Turing y la diagonalizaci\'on de Cantor dentro de un microchip de silicio.
\end{itemize}

\begin{objetivos}
\begin{itemize}
  \item Dise\~nar las redes sim\'etricas Pull-Up y Pull-Down requeridas para sintetizar el operador l\'ogico NAND.
  \item Conectar expl\'icitamente la abstracci\'on matem\'atica de la completitud funcional con la arquitectura f\'isica del transistor.
\end{itemize}
\end{objetivos}

\begin{lecturas}
\begin{itemize}
  \item[$(\star)$] \textbf{Sedra, A. S., \& Smith, K. C.} --- \emph{Microelectronic Circuits}, 7.a ed. Cap\'itulo~14 \S14.1 (L\'ogica CMOS Digital).
...
\begin{itemize}
  ...
  \item \textbf{Nagel, T.} --- ``What Is It Like to Be a Bat?''. \textit{The Philosophical Review}, 1974.
  \item \textbf{Feynman, R. P.} --- \emph{Feynman Lectures on Computation}. 
\
